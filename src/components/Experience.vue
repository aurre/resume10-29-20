<template>
  <section id="experience" class="resume-section p-3 p-lg-5 d-flex align-items-center">
    <div class="w-100">
      ...
      <div class="resume-item d-flex flex-column flex-md-row justify-content-between mb-5" v-for="(experience, index) in experienceList" :key="index">
        ...
        <div class="resume-date text-md-right">
          <span class="text-primary" v-for="(date, index) in experience.dates" :key="index">{{ date.start }} - {{ date.end }}</span>
        </div>
      </div>
    </div>
  </section>
    ...
    return {
      experienceList:  [
        ...
        },
        {
          ...
          "summary": "Podcasting operational change management inside of workflows to establish a framework. Taking seamless key performance indicators offline to maximise the long tail. Keeping your eye on the ball while performing a deep dive on the start-up mentality to derive convergence on cross-platform integration.",
          "dates": [
            {
              "start": "July 2010",
              "end": "December 2011"
